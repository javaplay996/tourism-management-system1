      ...
      <el-row>
        ...
        <el-form-item class="input" v-if="type!='info'"  label="方案编号" prop="fanganbianhao">
          <el-input v-model="ruleForm.fanganbianhao" 
              placeholder="方案编号" clearable  :readonly="ro.fanganbianhao"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="方案编号" prop="fanganbianhao">
              <el-input v-model="ruleForm.fanganbianhao" 
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `lvyougoumai/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.lvyougoumaiCrossAddOrUpdateFlag = false;
                  this.parent.search();
